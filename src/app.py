...
Uses persistent database storage with SQLAlchemy.
"""
...
def seed_initial_data(db: Session):
    """Seed database with initial activities if not already present"""
    ...
    # Create activities and students
...
def signup_for_activity(activity_name: str, email: str, db: Session = Depends(get_db)):
    ...
    existing_signup = db.query(Activity).filter(
        Activity.name == activity_name,
        Activity.participants.any(Student.email == email)
    ).first()
...
def unregister_from_activity(activity_name: str, email: str, db: Session = Depends(get_db)):
    ...
    student = db.query(Student).filter(
        Student.email == email,
        Student.activities.any(Activity.name == activity_name)
    ).first()
